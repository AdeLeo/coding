...
int nbreak(int p, int dir)
{
    ...
    /*We use "n<len" to cut off loops thar go around the whole necklace*/
    for(n = 0; n<len; n++, i=mod(i+dir, len))
    {
        ...
        /*
         If we've chosen a color and see a bead not white and not that color, stop
         */
...
void main(void)
{
    ...
    /*
     If the whole necklace can be gotten with a good break, we'll sometimes count beads more tan once.
     This can only happen when the whole necklace can be taken, when beads that can be grabbed from 
